...
def get_gspread_client():
    try:
        ...
        # This parses the string from your Secrets into a format gspread understands
...
if page == "📊 Dashboard":
    ...
    search_sn = st.text_input("🔍 Search SN", placeholder="Enter SN...")
    ...
    if search_sn:
        ...
        if not res.empty:
            ...
            col1.metric("Events Attended", len(hist))
            col2.metric("Total Mins", f"{int(hist[dur_col].sum())}")
            st.dataframe(hist, use_container_width=True, hide_index=True)
        else:
            st.warning("SN not found.")
    else:
        st.dataframe(df_staff[['SN', 'Rank', 'Name', 'Unit', 'Contact', 'Leader Badge']], use_container_width=True, hide_index=True)

elif page == "🗓️ Event Logs":
    st.title("🗓️ Event Logs")
    s_loc = st.text_input("🔍 Search Location")
